...
- **`snake-game-be/`** - FastAPI backend (Python 3.7+)
- **`snake-game-fe/`** - React frontend (TypeScript, Vite)
...
- Backend uses a Python virtual environment at `snake-game-be/venv/`
- Frontend uses npm packages in `snake-game-fe/node_modules/`
- Always activate the backend venv before running Python commands: `source snake-game-be/venv/bin/activate`
...
- Database migrations are managed with Alembic in `snake-game-be/alembic/`
- Environment variables are configured in `.env` files (see README.md for details)

### Navigating the Codebase
- Backend API routes: `snake-game-be/app/api/v1/`
- Backend models: `snake-game-be/app/models/`
- Backend schemas: `snake-game-be/app/schemas/`
- Frontend components: `snake-game-fe/src/components/`
- Frontend services: `snake-game-fe/src/services/`
- Frontend tests: `snake-game-fe/src/**/__tests__/` and `snake-game-fe/e2e/`

## Testing Instructions

### Backend Tests
- Run backend tests: `make test-be` or `cd snake-game-be && pytest`
- Tests are located in `snake-game-be/tests/`
- Ensure virtual environment is activated before running tests
- All tests should pass before committing

### Frontend Unit Tests
- Run frontend unit tests: `make test-fe` or `cd snake-game-fe && npm test`
- Tests use Vitest and React Testing Library
- Run in watch mode: `cd snake-game-fe && npm run test:watch`
- Run with coverage: `cd snake-game-fe && npm run test:coverage`

### E2E Tests
- Run E2E tests: `make test-e2e` (automatically starts servers)
- E2E tests use Playwright and are in `snake-game-fe/e2e/`
...
### Creating Migrations
- Navigate to backend: `cd snake-game-be`
...
- Or directly: `cd snake-game-be && alembic upgrade head`
- In Docker: `make docker-migrate`

### Rolling Back
- Downgrade one revision: `make migrate-down`
- Or directly: `cd snake-game-be && alembic downgrade -1`
...
2. **Check TypeScript compilation**: `cd snake-game-fe && npm run build`
...
1. Add route handler in `snake-game-be/app/api/v1/`
2. Add Pydantic schema in `snake-game-be/app/schemas/`
3. Add database model if needed in `snake-game-be/app/models/`
4. Create migration if model changed: `alembic revision --autogenerate -m "add new model"`
5. Add tests in `snake-game-be/tests/`
6. Update OpenAPI spec (auto-generated, but verify)

### Adding a New Frontend Component
1. Create component in `snake-game-fe/src/components/`
2. Add CSS file if needed (same directory)
3. Add tests in `__tests__/` subdirectory
4. Update routing in `snake-game-fe/src/App.tsx` if needed

### Updating Dependencies
- Backend: Edit `snake-game-be/requirements.txt`, then `cd snake-game-be && venv/bin/pip install -r requirements.txt`
- Frontend: Edit `snake-game-fe/package.json`, then `cd snake-game-fe && npm install`

## Environment Variables

### Backend (`.env` in `snake-game-be/`)
- `DATABASE_URL` - Database connection string
- `SECRET_KEY` - JWT secret key
- `ACCESS_TOKEN_EXPIRE_MINUTES` - Token expiration time
- `SESSION_TIMEOUT` - Session timeout in seconds

### Frontend (`.env` in `snake-game-fe/`)
